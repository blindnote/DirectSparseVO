#ifndef DIRECTSPARSEVO_NLLSSOLVERIMPL_H
#define DIRECTSPARSEVO_NLLSSOLVERIMPL_H
...
void NLLSSolver<D, T>::optimize(ModelType& model)
{
    ...
    else if ( method_ == Method::LevenbergMarquardt )
    {
...
void NLLSSolver<D, T>::optimizeGaussNewton(ModelType& model)
{
    ...
    for( iter_ = 0; iter_ < n_iter_; ++iter_)
    {
        ...
        // stop when coverged, i.e. update step too small
    ...
    for (iter_ = 0; iter_ < n_iter_; ++iter_)
    {
        ...
        do
        {
            ...
            if (rho_ > 0)
            {
                ...
                mu_ *= std::max(1./3, std::min(1. - pow(2*rho_-1, 3), 2./3.));
                nu_ = 2.;
                if (verbose_)
                {
                    std::cout << "It. " << iter_
...
template <int D, typename T>
inline const Eigen::Matrix<double, D, D>& NLLSSolver<D, T>::getInformationMatrix() const
{
    return H_;
}
